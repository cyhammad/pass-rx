...
import DisciplineDropdowns from "../dropdowns/DisciplineDropdowns";

const FlashCard = ({ toggleFlash ,title,buttonTitle}) => {
  const closeModal = () => {
    toggleFlash();
  };
  return (
      ...
      <div className="z-50  flex h-[352px] md:h-[502px] w-[700px] flex-col items-center   rounded-lg  bg-white px-5 shadow-md md:px-10">
        <div className="mt-4 flex w-full items-center  justify-between md:mt-11 ">
          ...
          <div className="flex  w-[62%] sm:w-[42%] md:gap-3">
            ...
            <button
              className=" w-full rounded-lg border border-primary   text-xs  sm:text-sm  font-medium text-primary focus:outline-none"
              onClick={() => closeModal()}
            >
              {buttonTitle}
            </button>
        ...
        <div className=" md:mt-14 mt-3 flex w-full flex-col justify-between h-full ">
          <div className="relative md:w-[247px] w-full items-center justify-center gap-x-2 rounded-lg border border-border-color px-4 py-3 ">
            <DisciplineDropdowns/>
            <label
