...
const AddQuestionModal = ({ setShowModal, quizbank, token }) => {
  const [selectedTab, setSelectedTab] = useState("Question");
  const [selectedDiscipline, setSelectedDiscipline] = useState(
    quizbank.disciplines[0],
  );
  const [difficulty, setDifficulty] = useState("Easy");
  const [includeFlashFacts, setIncludeFlashFacts] = useState(false);
  ...
  return (
    ...
    >
      ...
      ></div>
      <div className="z-50 flex h-full max-h-[80vh] w-full max-w-[700px] flex-col gap-8 overflow-auto rounded-lg bg-mattBlack p-6 shadow-md">
        ...
        <div className="relative flex w-fit flex-col pb-4">
          ...
          <motion.div
            initial={{ left: "30px" }}
            animate={{
              left: selectedTab === "Question" ? "30px" : "calc(50% + 35px)",
            }}
            transition={{ duration: 0.3 }}
